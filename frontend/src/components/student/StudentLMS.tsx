...
export const StudentLMS: React.FC = () => {
  ...
  useEffect(() => {
    let active = true;
    dataService.getLmsLessons().then((data) => {
      if (!active) return;
      setLessons(data || []);
      setLoading(false);
    });
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {loading ? (
        <Card className="p-8 text-center text-text-tertiary">Loading digital classroom lessons...</Card>
      ) : courses.length === 0 ? (
        <Card className="p-8 text-center text-text-tertiary">
          <BookOpen className="mx-auto mb-2 opacity-40" size={32} />
          <p className="font-semibold text-foreground">No digital classroom lessons available yet.</p>
          <p className="text-micro text-text-secondary mt-1">Course materials published by teachers will appear here.</p>
        </Card>
      ) : (
              ...
              >
                ...
                <div className="flex flex-col gap-2.5">
                  {courseLessons.map((lesson) => {
                    const meta = contentMeta[lesson.contentType] || contentMeta.notes;
                    return (
                      <Card
                        key={lesson.id}
                        className={cn(
